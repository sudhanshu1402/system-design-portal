...
Enqueueing costs one Redis write. The API answers with `202` and a job id as soon as that write returns, and the slow work moves to a pool you can scale on queue depth instead of on HTTP traffic.
